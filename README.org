...
** Bots
     ...
     | https://t.me/Alessandro0rlandoBot | [[https://www.instagram.com/alessandro_orlando_official/][Alessandro Orlando]]               |
     ...
     | https://t.me/VittorioSgarbiBot    | [[https://it.wikipedia.org/wiki/Vittorio_Sgarbi][Vittorio Sgarbi]]                  |
...
**** GUI tool (Replies Editor)
     ...
     To edit a bot’s ~*_replies.json~ via a GUI (and auto-regenerate ~*_triggers.md~ on save), use the [[modules/repliesEditorServer/README.md][Replies Editor (GUI)]].
...
*** Long Polling
  ...
  #+begin_src sh
    ...
    /home/benkio/Mega/sBots/RichardPHJBensonBot/src/main/resources/rphjb_RichardPHJBensonBot.token
...
**** Localhost
     ...
     - run ~sbt assembly~
     - run ~java -cp main/target/scala-3.3.8/main.jar com.benkio.main.MainWebhook~
...
** Healthcheck
  ...
  - **Config**: In ~modules/main/src/main/resources/application.conf~
    ...
    - ~endpoint~ :: URL to POST (e.g. ~https://hc-ping.com/<your-check-uuid>~).
...
*** GitHub Deploy Workflow (current target)

  The automated deploy workflow in ~.github/workflows/deploy.yml~ targets:

  - host: ~DEPLOY_HOST~
  - ssh port: ~DEPLOY_PORT~
  - user: ~DEPLOY_USER~
  - server OS/package manager: Alpine Linux (~apk~)
  - remote deploy directory: ~DEPLOY_DIR~
  - webhook public host/IP: ~WEBHOOK_PUBLIC_HOST~
  ...
  - ~SSH_PRIVATE_KEY~ :: private key used by GitHub Actions to connect via SSH
  - ~DEPLOY_SUDO_PASSWORD~ :: password used for ~sudo~ while installing/updating runtime packages
  - bot tokens (eg ~ABAR_TOKEN~, ~CALA_TOKEN~, etc.) as already used by the workflow

  Notes:

  - The deploy and webhook endpoints are configured through repository variables, so no host/IP value needs to stay hardcoded in the workflow file.
  - ~webhook-certificate~, ~keystore-path~, and ~keystore-password~ are optional. If omitted, the app can run behind a reverse proxy that terminates TLS.
  - For better security, prefer adding the server fingerprint to ~known_hosts~ instead of using ~StrictHostKeyChecking=no~.
...
*** Deploy
 ...
 - Set the ~main/src/resources/application.conf~ accordingly:
   ...
   - ~webhook-certificate~ (optional) with the path of the public certificate, eg ~sbotsCertificatePub.pem~
   - ~keystore-path~ (optional) add the path to the keystore, eg ~sbotsKeystore.jks~
   - ~keystore-password~ (optional) add the password of the keystore, eg ~sbotsKeystorePassword~
   - If TLS is terminated by a reverse proxy (nginx/caddy/traefik), leave the three options above unset and expose only the internal app port to the proxy.
 - Run ~sbt "clean; main/assembly"~ to create the fat jar
 - Move the fat jar to the VM by ~rsync~ and ~ssh~. Eg. ~rsync -P -e "ssh -i <<private key of the vm>>" <<path to the fat jar -> sBots/main/target/scala-3.3.8/main.jar>> <<user of the vm>>@<<public IP of the vm>>:/home/<<user of the vm>>/main.jar~ (~rsync -P -e "ssh -i ubuntu_rsa.pem" /home/benkio/workspace/sBots/main/target/scala-3.3.8/main.jar ubuntu@129.152.27.196:/home/ubuntu/bots/main.jar~)
