...
**** Webhook Extra Configuration
  ...
  - Convert the keystore to PEM :: following the commands in the link above. eg:
    ...
    #+end_src
  - Check your application.conf values :: be sure to specify the certificate and the keystore name and password correctly.
...
*** Deploy
 ...
 - Set the ~main/src/resources/application.conf~ accordingly:
   ...
   - ~webhook-certificate~ with the path of the public certificate, eg ~certificatePublic.pem~
