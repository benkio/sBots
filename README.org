...
*** General(Maybe) Development
    ...
    - [ ] Refactor logging
    - [ ] Feedback button  on bot messages or command
    ...
    - [ ] Move this section in the Issues
    - [X] Collect statistics on the files + Add command to see the top 20 matches
      - [X] Database Table count column
      - [X] Change botDB to not overwrite the count
      - [X] Code to accumulate the count on trigger
      - [X] New Command to get the top 20 files triggered
**** GIF Refactoring
