...
** Add Data to the Bots
     ...
     To add data to the bots you need to add the file name and
     downloadable link (usually Dropbox), to the json at the root of
     each bot folder. Then modify the scala code to add the type that
     creates the mappings between the triggers and the files. See
     planty of examples of any kind throughout the code, especially
     under RichardPHJBensonBot's data package.
