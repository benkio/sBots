...
**** Webhook Extra Configuration
  ...
  - Server Prerequisites :: Check [[https://core.telegram.org/bots/webhooks#the-short-version][this page]] for the prerequisites required by telegram to work with webhook. In our case we choose the port 8443
  ...
  - Change Certificates Permissions and Owner :: This can be done by running ~sudo chown root:root <<Certificate>>~ and ~sudo chmod 644 <<Certificate>~. Do this on both private and public keys.
  ...
  - Allow the new Certificates :: Run ~audit2allow -a -M mycertwatch~ and then ~semodule -i mycertwatch.pp~. [[https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/6/html/security-enhanced_linux/sect-security-enhanced_linux-fixing_problems-allowing_access_audit2allow][Reference]]
...
*** Deploy
 ...
 - Set the ~main/src/resources/application.conf~ accondigly:
    ...
    Then move the jar in to the server.
