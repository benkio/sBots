...
object CalandroBot extends BotSkeleton {
  ...
  override lazy val messageRepliesData : List[ReplyBundleMessage] = List(
    ...
    ReplyBundleMessage(TextTrigger(List("gay", "frocio", "culattone", "ricchione"       )), text = TextReply((m : Message) => List("CHE SCHIFO!!!"), false)),
    ReplyBundleMessage(TextTrigger(List("caldo", "scotta"                               )), text = TextReply((m : Message) => List("Come i carbofreni della Brembo!!"), false)),
    ReplyBundleMessage(TextTrigger(List("ciao", "buongiorno", "salve"                   )), text = TextReply((m : Message) => List("Buongiorno Signori"), false)),
    ReplyBundleMessage(TextTrigger(List("film"                                          )), text = TextReply((m : Message) => List("Lo riguardo volentieri"), false)),
    ReplyBundleMessage(TextTrigger(List("stasera", "?"                                  )), text = TextReply((m : Message) => List("Facciamo qualcosa tutti assieme?"), false), matcher = ContainsAll),
    ReplyBundleMessage(TextTrigger(List(" hd", "nitido", "nitidezza", "alta definizione")), text = TextReply((m : Message) => List("Eh sì, vedi...si nota l'indecisione dell'immagine"), false)),
    ReplyBundleMessage(TextTrigger(List("qualità"                                       )), text = TextReply((m : Message) => List("A 48x masterizza meglio"), false)),
    ReplyBundleMessage(TextTrigger(List("macchina", "automobile"                        )), text = TextReply((m : Message) => List("Hai visto l'ultima puntata di \"Top Gear\"?"), false)),
    ReplyBundleMessage(TextTrigger(List(" figa ", " fregna ", " gnocca ", " patacca "   )), text = TextReply((m : Message) => List("Io so come fare con le donne...ho letto tutto..."), false)),
    ReplyBundleMessage(TextTrigger(List("ambulanza", emoji":ambulance:"                 )), text = TextReply((m : Message) => List(emoji":triumph: :horns_sign: :hand_with_index_and_middle_fingers_crossed: :hand_with_index_and_middle_fingers_crossed: :horns_sign: :triumph:"), false)),
    ReplyBundleMessage(TextTrigger(List("pc", "computer"                                )), text = TextReply((m : Message) => List("Il fisso performa meglio rispetto al portatile!!!"), false)),
    ReplyBundleMessage(TextTrigger(List("videogioc", emoji":video_game:"                )), text = TextReply((m : Message) => List(s"GIOCHI PER IL MIO PC #${Random.nextInt(Int.MaxValue)}??No ma io non lo compro per i giochi!!!"), false)),
    ReplyBundleMessage(TextTrigger(List(" hs", "hearthstone"                            )), text = TextReply((m : Message) => List("BASTA CON QUESTI TAUNT!!!"), false)),
