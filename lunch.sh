...
then
    ...
    then
        ...
        (cd ./modules/main/target/scala-3.7.2/; java -cp main.jar com.benkio.main.MainPolling)
    else
        echo "-------------------------Run Webhook Bots-------------------------"
        cp ./botDB.sqlite3 ./modules/main/target/
        (cd ./modules/main/target/scala-3.7.2/; java -cp main.jar com.benkio.main.MainWebhook)
    fi
