...
class YouTubeServiceMock(
    ...
    onSaveCaption: (String, Path, String) => IO[Option[Path]] = (_, _, _) =>
      IO.raiseError(Throwable("[YouTubeServiceMock] Unexpected Call"))
  ...
  override def saveCaption(videoId: String, captionFolderPath: Path, captionLanguage: String): IO[Option[Path]] =
