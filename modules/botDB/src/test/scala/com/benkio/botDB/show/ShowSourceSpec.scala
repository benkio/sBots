...
class ShowSourceSpec extends FunSuite {
  ...
  test("fromString should return the input url if it's valid playlist") {
    ...
    assertEquals(ShowSource[EitherThrow](url, botName, outputFilePath), Right(YoutubePlaylist(Uri.unsafeFromString(url),botName, outputFilePath)))
  }
  test("fromString should return the input url if it's valid channel") {
    val url = "https://www.youtube.com/@youtuboancheio1365"
    assertEquals(ShowSource[EitherThrow](url, botName, outputFilePath), Right(YoutubeChannel(Uri.unsafeFromString(url),botName, outputFilePath)))
  }
