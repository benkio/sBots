  ...
  test("TokenReader.token should return TokenNotFound in case the filename is not found") {
    ...
    val repository = RepositoryMock(
      getResourceFileHandler = _ =>
        IO.pure(Left(Repository.RepositoryError.NoResourcesFoundFile(Document(tokenFilename))))
    )
    val actual: IO[Either[Throwable, String]] = TokenReader.token[IO](
  ...
  test("TokenReader.token should return TokenEmpty in case the file is empty") {
    ...
    val actual: IO[Either[Throwable, String]] = for {
      ...
      repository = RepositoryMock(
        getResourceFileHandler = _ =>
          IO.pure(Right(NonEmptyList.one(MediaResourceFile(Resource.pure(tokenPath)))))
      )
      result <- TokenReader.token[IO](
        tokenFilename = tokenFilename,
        repository = repository
      ).use(IO.pure).attempt
