...
/** Test fixture that builds a real BotSetup as close as possible to main: real DBLayer (from DBFixture),
  * DBRepository, DropboxClient, Ember httpClient, BackgroundJobManager, JsonRepliesRepository.
  * Uses a stub token and skips webhook delete.
  * Use `botSetupResource.use { botSetup => ... }` in tests.
  */
...
trait BotSetupFixture extends DBFixture { self: FunSuite =>
  ...
  lazy val botSetupFixture: FunFixture[BotSetupFixtureResources] = FunFixture[BotSetupFixtureResources](
    setup = testOptions => BotSetupFixture.fixtureSetup(this, botSetupFixtureConfig, testOptions)(using log),
...
object BotSetupFixture {
  ...
  /** Builds a Resource[IO, BotSetup[IO]] using the same building blocks as main: real DBLayer,
    * Ember httpClient, DropboxClient, DBRepository, BackgroundJobManager, JsonRepliesRepository, BotApi.
    */
