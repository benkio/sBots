...
trait DBTimeout[F[_]] {
  def getOrDefault(chatId: Long, botId: String): F[DBTimeoutData]
  def setTimeout(timeout: DBTimeoutData): F[Unit]
  def removeTimeout(chatId: Long, botId: String): F[Unit]
  def logLastInteraction(chatId: Long, botId: String): F[Unit]
}
...
object DBTimeout {
  ...
  private[db] class DBTimeoutImpl[F[_]: Async](transactor: Transactor[F], log: LogWriter[F]) extends DBTimeout[F] {
    ...
    override def getOrDefault(chatId: Long, botId: String): F[DBTimeoutData] =
    ...
    override def removeTimeout(chatId: Long, botId: String): F[Unit] =
      log.info(s"DB removing timeout for $chatId - $botId") *>
        removeTimeoutQuery(chatId, botId).run
          .transact(transactor)
          .void

    override def logLastInteraction(chatId: Long, botId: String): F[Unit] =
  ...
  def getOrDefaultQuery(chatId: Long, botId: String): Query0[DBTimeoutData] =
    sql"SELECT chat_id, bot_id, timeout_value, last_interaction FROM timeout WHERE chat_id = $chatId AND bot_id = $botId"
      .query[DBTimeoutData]
  ...
  def removeTimeoutQuery(chatId: Long, botId: String): Update0 =
    sql"DELETE FROM timeout WHERE chat_id = $chatId AND bot_id = $botId".update

  def logLastInteractionQuery(chatId: Long, botId: String): Update0 =
    sql"UPDATE timeout SET last_interaction = ${Instant.now().getEpochSecond()} WHERE chat_id = $chatId AND bot_id = $botId".update
}
