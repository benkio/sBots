...
class MosconiBotSpec extends CatsEffectSuite {
  ...
  val emptyDBLayer                  = DBLayerMock.mock(MosconiBot.botName)

  test("triggerlist should return the link to the trigger txt file") {
    val triggerlistUrl = MosconiBot
  ...
  test("the `mos_list.csv` should contain all the triggers of the bot") {
    ...
    val botFile = MosconiBot.messageRepliesData[IO].flatMap(_.mediafiles.map(_.filename))
  ...
  test("the `mos_triggers.txt` should contain all the triggers of the bot") {
    ...
    val botMediaFiles = MosconiBot.messageRepliesData[IO].flatMap(_.mediafiles.map(_.show))
    val botTriggersFiles =
      MosconiBot.messageRepliesData[IO].flatMap(mrd => Show[Trigger].show(mrd.trigger).split('\n'))
