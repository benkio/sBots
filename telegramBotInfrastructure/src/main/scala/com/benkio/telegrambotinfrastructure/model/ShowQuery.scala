...
final case class ShowQueryKeyword(
  ...
  maxDate: Option[LocalDate] = None) extends ShowQuery
...
object ShowQuery {
  ...
  implicit val deflocalDateQueryParamDecoder: QueryParamDecoder[LocalDate] = localDateQueryParamDecoder(DBShowData.dateTimeFormatter)
  ...
  def apply(queryString: String): ShowQuery = (queryString, Query.unsafeFromString(queryString)) match {
    ...
    case (input, query) if TitleKeywordsQueryParamMatcher.unapplySeq(query.multiParams).isEmpty => ShowQueryKeyword(titleKeywords = List(input))
    case (_, query) => ShowQueryKeyword(
      titleKeywords = TitleKeywordsQueryParamMatcher.unapplySeq(query.multiParams).map(_.toList).getOrElse(List.empty),
