    ...
    given telegramMp3Reply: TelegramReply[Mp3File] = new TelegramReply[Mp3File]{
      def reply[F[_]: Async : LogWriter: Api](
        ...
        TelegramReply.telegramFileReplyPattern[F](
          ...
          sendFileAPIMethod = (chatId, ifile, replyToMessageId) => Methods.sendAudio(
            chatId,
            ifile,
            replyToMessageId
          )
    ...
    given telegramGifReply: TelegramReply[GifFile] = new TelegramReply[GifFile]{
      def reply[F[_]: Async : LogWriter: Api](
        ...
        TelegramReply.telegramFileReplyPattern[F](
          ...
          mediaFile = reply,
          replyToMessage = replyToMessage,
          sendFileAPIMethod = (chatId, ifile, replyToMessageId) => Methods.sendAnimation(
            chatId,
            ifile,
            replyToMessageId
          )
    ...
    given telegramPhotoReply: TelegramReply[PhotoFile] = new TelegramReply[PhotoFile]{
      def reply[F[_]: Async : LogWriter: Api](
        ...
        TelegramReply.telegramFileReplyPattern[F](
          ...
          mediaFile = reply,
          replyToMessage = replyToMessage,
          sendFileAPIMethod = (chatId, ifile, replyToMessageId) => Methods.sendPhoto(
            chatId,
            ifile,
            replyToMessageId
          )
    ...
    given telegramVideoReply : TelegramReply[VideoFile] = new TelegramReply[VideoFile]{
      def reply[F[_]: Async : LogWriter: Api](
        ...
        TelegramReply.telegramFileReplyPattern[F](
          ...
          mediaFile = reply,
          replyToMessage = replyToMessage,
          sendFileAPIMethod = (chatId, ifile, replyToMessageId) => Methods.sendVideo(
            chatId,
            ifile,
            replyToMessageId
          )
