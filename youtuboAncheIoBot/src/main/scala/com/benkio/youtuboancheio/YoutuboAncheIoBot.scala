...
class YoutuboAncheIoBotMainPolling[F[_]: Parallel: Async: Api: LogWriter](
...
trait YoutuboAncheIoBot[F[_]] extends BotSkeleton[F] {
  ...
  override def commandRepliesDataF(implicit asyncF: Async[F], log: LogWriter[F]): F[List[ReplyBundleCommand[F]]] =
    ...
    ).sequence.map(cs => cs ++ YoutuboAncheIoBot.commandRepliesData[F])
...
object YoutuboAncheIoBot extends BotOps {
  ...
  def buildCommonBot[F[_]: Async](
    ...
    _ <- Resource.eval(
      Async[F].raiseWhen(deleteWebhookResponse.status != Status.Ok)(
        new RuntimeException("[YoutuboAncheIoBot] The delete webhook request failed: " + deleteWebhookResponse.as[String])
      )
    )
  ...
  def buildPollingBot[F[_]: Parallel: Async, A](
      action: YoutuboAncheIoBotMainPolling[F] => F[A]
    ...
    action(new YoutuboAncheIoBotMainPolling[F](httpClient_tk_ra._3))
  })
